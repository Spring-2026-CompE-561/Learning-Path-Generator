...
from app.schemas.learningpath import LearningPathCreate, LearningPath as LearningPathSchema
...
@api_router.post("/", response_model = LearningPathSchema, status_code = status.HTTP_201_CREATED)

def create_learning_path(
    data: LearningPathCreate, db: Session = Depends(get_db),
...
def get_using_specfic(
    learning_path_id: int, db: Session = Depends(get_db),
    current_user: UserModel = Depends(user_service.get_current_user),
):
...
def update_path(
    learning_path_id: int, updates: dict, db: Session = Depends(get_db),
    current_user: UserModel = Depends(user_service.get_current_user),
):
    return learning_path_service.update_learning_path(db, learning_path_id, current_user.id, updates)
