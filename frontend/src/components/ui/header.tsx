...
const nameOfPages = [
    ...
    //FIXME: A placeholder, need to dynamically change name according to the learning path pages
    {
        name:'Learning Path Name', path: '/learning-path'
    },
];
...
export function Header() {
    ...
    //logic to find current page name based on the pathname
    const currentPage = nameOfPages.find(page =>
    pathname.startsWith(page.path)
    );

    let pageTitle = currentPage?.name;

    if (!pageTitle && pathname.startsWith('/learning-path')) {
    ...
    if (pathname === '/'){
        ...
        return (
            <header className="w-full h-fit bg-background text-foreground">   
                <div className="w-full h-fit flex items-center justify-between p-10 flex-col sm:flex-row bg-background text-foreground">   
                    <div className="w-fill h-fit flex justify-end items-center gap-2">
                        <svg className="w-25 h-25" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024">
                            <title>Learning Path Logo traced</title>
